...
def build_controllerUI():
	...
	root = m
	control_frm = tk.Frame(root,bg="Black",borderwidth=100,border=5)
	...
	# homing function
	homing = tk.Button(control_frm, text='🏠︎',width=2,justify="right", command=lambda : head_home()) ; homing.grid(row = 7, column = 1)
	...
	increment_label = ttk.Label(control_frm, text="Input the Increment Size:", style='TLabel')
	increment_label.grid(row = 3, column = 0, padx=5, pady=5)
	i=4
	for increments in increment_options:
		r = ttk.Radiobutton(
	...
	set_target_z = tk.Button(btn_frm, text='SET SURFACE Z', width=20, command=lambda : set_target_z_position()) ; set_target_z.grid(row=10, column=1, padx=5, pady=5)
	...
	points_label.grid(row = 9, column = 8, padx=5, pady=5)
	set_point1 = tk.Button(btn_frm, text='SET Point', width=20, command=lambda : set_a_point(points_label))
	...
	set_point.config(command=lambda : set_point_mode(set_point,set_point1,points_label))
	set_params = tk.Button(btn_frm,text="Set Params",width=20,command=lambda:open_param_menu()) ; set_params.grid(row=11,column=8, padx=5, pady=5)
...
def build_param_menu():
	...
	set_mode = tk.Button(param_frm, text='Current Mode', width=20, relief='sunken') ; set_mode.grid(row=1, column=8, padx=5, pady=5)

	
    #setting commmand for current mode/voltage mode button
	set_mode.config(command=lambda : set_mode_electroplating(set_mode,input_current,set_current,input_voltage,set_voltage))
	
	start_btn = tk.Button(param_frm,text="▶ START ELECTOPLATING!",width=20,command=lambda : do_task(cur_label,vol_label,tar_vol_label,time_remaining_label))
	start_btn.grid(row=11,column=8)

	returnbtn = tk.Button(param_frm,text="Return to\nController",width=20,command=lambda: open_controller()); returnbtn.grid(column=0,row=11)
